...
def unit_test(rnn_cell, rnn, irnn, input, h0, c0, batch_first, bidirectional, seq_input, res_idx):
    print("\n------------------- nn.LSTMCell -------------------")
    # run LSTMCell forward model
    hn_f = h0[0]
    cn_f = c0[0]
    ...
    for i in range(seq_length):
        if batch_first == True:
            hn_f, cn_f = rnn_cell(input[:,i,:], (hn_f, cn_f))
        else:
            hn_f, cn_f = rnn_cell(input[i,:,:], (hn_f, cn_f))
        # print("hn_f: ", hn_f)
        if i == 0:
    ...
    if bidirectional == True:
        ...
        cn_f = cn_f.unsqueeze(0)
        print("hn_f: ", list(hn_f.size()))
        print("cn_f: ", list(cn_f.size()))
        # print(cell_output_f)
    else:
        cell_output = cell_output_f
        print("cell_output: ", list(cell_output.size()))
        cell_hn = hn_f.unsqueeze(0)
        cell_cn = cn_f.unsqueeze(0)
        print("cell_hn: ", list(cell_hn.size()))
        print("cell_cn: ", list(cell_cn.size()))
        # print(cell_output)
        # print(cell_hn)
        # print(cell_cn)

    if bidirectional == True:
        # run LSTMCell forward reverse model
        hn_b = h0[1]
        cn_b = c0[1]
        for i in range(seq_length):
            if batch_first == True:
                hn_b, cn_b = rnn_cell(input[:,seq_length-i-1,:], (hn_b, cn_b))
            else:
                hn_b, cn_b = rnn_cell(input[seq_length-i-1,:,:], (hn_b, cn_b))
            # print("hn_b: ", hn_b)
            if i == 0:
    ...
    print(torch.all((cell_cn - cn) < 0.000001))
    ...
    ioutput, (ihn, icn) = irnn(input, (h0, c0))
    print("ioutput: ", list(ioutput.size()))
    print("ihn: ", list(ihn.size()))
    print("icn: ", list(icn.size()))
    # print(ioutput)
    # print(ihn)
    # print(icn)
    ...
    print(torch.all((icn - cn) < 0.000001))

    print("\n------------------- nb.LSTM -------------------")
    ioutput2, (ihn2, icn2) = irnn(input)
    print("ioutput2: ", list(ioutput2.size()))
    print("ihn2: ", list(ihn2.size()))
    print("icn2: ", list(icn2.size()))
    # print(ioutput2)
    # print(ihn2)
    # print(icn2)
    ...
    print(torch.all((icn2 - cn) < 0.000001))

    print("\n------------------- nn.LSTM sequences -------------------")
    output, (hn, cn) = rnn(seq_input, None)
    ...
    print("\n------------------- nb.LSTM sequences -------------------")
    ioutput, (ihn, icn) = irnn(seq_input, None)
...
def benchmark(rnn, irnn, input, h0, c0, seq_input):
    loops = 100
    print("\n------------------- Perf nn.LSTM -------------------")
    output, (hn, cn) = rnn(input, (h0, c0))
    ...
    latency = total / loops
    print("nn.LSTM latency: ", latency, " ms")

    print("\n------------------- Perf nb.LSTM -------------------")
    output, (hn, cn) = irnn(input, (h0, c0))
    start_time = time.time()
    for _ in range(loops):
        output, (hn, cn) = irnn(input, (h0, c0))
    end_time = time.time()
    total = (end_time - start_time) * 1000
    latency = total / loops
    print("nb.LSTM latency: ", latency, " ms")

    print("\n------------------- Perf nn.LSTM sequences -------------------")
    output, (hn, cn) = rnn(seq_input, None)
    start_time = time.time()
    for _ in range(loops):
        output, (hn, cn) = rnn(seq_input, None)
    ...
    total = (end_time - start_time) * 1000
    latency = total / loops
    print("nn.LSTM sequences latency: ", latency, " ms")

    print("\n------------------- Perf nb.LSTM sequences -------------------")
    output, (hn, cn) = irnn(seq_input, None)
    start_time = time.time()
    for _ in range(loops):
        output, (hn, cn) = irnn(seq_input, None)
    end_time = time.time()
    total = (end_time - start_time) * 1000
    latency = total / loops
    print("nb.LSTM sequences latency: ", latency, " ms")

    # # jit script model
    # script_irnn = torch.jit.script(irnn)
    # torch.jit.save(script_irnn, 'lstm.torchscript.pt')
    # script_irnn = torch.jit.load('lstm.torchscript.pt')

    # output, (hn, cn) = script_irnn(input, (h0, c0))
    # start_time = time.time()
    # for _ in range(loops):
    #     output, (hn, cn) = script_irnn(input, (h0, c0))
    # end_time = time.time()
    # total = (end_time - start_time) * 1000
    # latency = total / loops
    # print("scripted nb.LSTM latency: ", latency, " ms")
...
def generate_inputs(batch_size, sequence_length, input_size, hidden_size, num_layers, batch_first, bidirectional):
    ...
    c0 = torch.zeros(D*num_layers, batch_size, hidden_size)
    print("input : ", list(input.size()))
    print("hidden: ", list(h0.size()))
    print("state : ", list(c0.size()))
    ...
    return input, h0, c0, seq_input, res_idx


def create_LSTM(input_size, hidden_size, num_layers, batch_first, bidirectional):
    rnn_cell = nn.LSTMCell(input_size, hidden_size)
    rnn = nn.LSTM(input_size, hidden_size, num_layers, batch_first=batch_first, bidirectional=bidirectional)
    irnn = nb.LSTM(input_size, hidden_size, num_layers, batch_first=batch_first, bidirectional=bidirectional)
...
def run(input_size, hidden_size, num_layers, batch_first, bidirectional, batch_size, sequence_length, BM):
    ...
    rnn_cell, rnn, irnn = create_LSTM(input_size, hidden_size, num_layers, batch_first, bidirectional)

    # inputs
    print("batch_size: ", batch_size)
    print("seq length: ", sequence_length)
    input, h0, c0, seq_input, res_idx = generate_inputs(batch_size, sequence_length, input_size, hidden_size, num_layers, batch_first, bidirectional)

    if BM == False:
        unit_test(rnn_cell, rnn, irnn, input, h0, c0, batch_first, bidirectional, seq_input, res_idx)
    else:
        benchmark(rnn, irnn, input, h0, c0, seq_input)
